...
class HomeController extends CommonController {
     ...
     * @author bigfoot
     */
    protected function is_visit() {
        //用户访问密码输入检测
        $visit_auth = session('visit_auth');
        if ($visit_auth) {
            return $visit_auth;
        } else {
            Cookie('__forward2__', 'http://'.$_SERVER['HTTP_HOST'].$_SERVER['PHP_SELF'].'?'.$_SERVER['QUERY_STRING']);
            $this->redirect('User/User/visit');
        }
    }
    /**
     * 禁止用户财务控制
    ...
    protected function is_disable() {
        $uid = is_login();
        $map['uid'] = $uid;
        $status = D('User/Caiwu')->where($map)->getField('status');
        if ($status > 0 ) {
            return true;
        } else {
            $this->error('您已经被冻结财务操作，请联系管理员处理');
        }
    }
