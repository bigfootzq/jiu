...
function parse_content($str) {
    ...
    if (C('STATIC_DOMAIN')) {
        ...
        return preg_replace('/<img.*?src="(.*?Uploads.*?)"(.*?)>/i', "<img class='lazy lazy-fadein img-responsive' style='display:inline-block;' data-src='$1'>", $str);
    } else {
...
function get_user_id($username) {
    ...
    if ($username != '' && !empty($username)){
        ...
        $userid = $user_onject->where($map)->getField('id');
    }else{
        return false;
    }
    if ($userid) {
...
function get_user_name($userid) {
    ...
    $username = $user_onject->where($map)->getField('username');
 ...
 * @author bigfoot
 */
function get_user_caiwu_info($id, $field) {
    if (D('Admin/Module')->where('name="User" and status="1"')->count()) {
        $user_onject= D('User/Caiwu');
    } else {
        $user_onject= D('Admin/User');
    }
    $usercaiwuinfo = $user_onject->find($id);
    if (!$field) {
        return $usercaiwuinfo;
    }
    if ($usercaiwuinfo[$field]) {
        return $usercaiwuinfo[$field];
    } else {
        return false;
    }
}
