...
class UserController extends HomeController {
    ...
    public function login() {
        if (IS_POST) {
            ...
            if ($uid) {
                ...
                $this->success('登录成功！', Cookie('__forward__') ? U('Center/index') : U('Center/index'), array('data' => D('User/User')->detail($uid)));
            ...
            if (is_login()) {
                $this->error("您已登陆系统", Cookie('__forward__') ? U('Center/index') : U('Center/index'));
            }
    ...
    public function visit() {
        if(IS_POST){
            $visitpassword = I('post.visitpassword');
            // dump($visitpassword );
            if ($visitpassword){
                $t_object = D('User/Caiwu');
                $visit_auth = $t_object->visitpassword_auth($visitpassword);
                // dump($visit_auth);
                if ($visit_auth){
                    session('visit_auth',is_login());
                    // dump(Cookie('__forward2__'));  
                    redirect( Cookie('__forward2__') );//跳转到上次点击页面
                }else{
                    $this->error('访问失败,密码错误');
                }
            }else{
                $this->error('请填写访问密码');
            }
            ...
            $builder->setMetaTitle('访问密码验证')
            ->addFormItem('visitpassword', 'password', '访问密码')
            ->setPostUrl(U('visit')) 
            ->setAjaxSubmit(false)
    ...
    public function sendMailVerify(){
        ...
        $mail_data['content'] = '尊敬的用户您好：<br>如果您正使用该邮箱'.I('post.email').'【注册/修改密码】，请在验证码输入框中输入：
