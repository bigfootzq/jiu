...
class CenterController extends HomeController {
    ...
    public function index() {
        ...
        // $user_caiwu_info = get_user_caiwu_info($uid);
        $user_caiwu_info = D('InfoView')->find($uid);
        // dump($user_caiwu_info);
        ...
        $this->assign('user_caiwu_info', $user_caiwu_info);
    ...
    public function password() {
        ...
        if (IS_POST) {
            ...
            $tab_list = array(
                                '0' => array(
                                    'title' => '登陆密码',
                                    'href'   => U('User/Center/password'),
                                ),
                                '1' => array(
                                    'title' => '访问密码',
                                    'href'   => U('User/Center/password2'),
                                ),
                                '2' => array(
                                    'title' => '支付密码',
                                    'href'   => U('User/Center/password3'),
                                )
                            );
            // dump($tab_list);
            $builder = new \Common\Builder\FormBuilder();
            $builder->setMetaTitle('修改登陆密码')  // 设置页面标题
                    ->setTabNav($tab_list,0)  // 设置Tab按钮列表 
                    ->setPostUrl(U(''))        // 设置表单提交地址
                    ->addFormItem('password', 'password', '旧登陆密码')
                    ->addFormItem('newpassword', 'password', '新登陆密码')
                    ->addFormItem('repassword', 'password', '重复新登陆密码')
                    ->setTemplate(C('USER_CENTER_FORM'))
                    ->display();
    ...
    public function password2() {
        ...
        if (IS_POST) {
            ...
            if ($data) {
                $password = user_md5(I('password'));
                $newpassword = user_md5(I('newpassword'));
                if ($password === get_user_caiwu_info($uid, 'visitpassword')) {
                    $result = $user_object->where(array('uid' => $uid))
                            ->setField('visitpassword', $newpassword);
                    if ($result) {
                        $this->success('访问密码修改成功', U(''));
                    } else {
                        $this->error('访问密码修改失败'.$user_object->getError());
                    }
                } else {
                    $this->error('旧密码输入错误');
                }
            ...
            // dump($tab_list);
            $builder = new \Common\Builder\FormBuilder();
            $builder->setMetaTitle('修改访问密码')  // 设置页面标题
                    ->setTabNav($tab_list,1)  // 设置Tab按钮列表 
                    ->setPostUrl(U(''))        // 设置表单提交地址
                    ->addFormItem('password', 'password', '旧访问密码')
                    ->addFormItem('newpassword', 'password', '新访问密码')
                    ->addFormItem('repassword', 'password', '重复新访问密码')
                    ->setTemplate(C('USER_CENTER_FORM'))
                    ->display();
    ...
    public function password3() {
        ...
        if (IS_POST) {
            ...
            if ($data) {
                $password = user_md5(I('password'));
                $newpassword = user_md5(I('newpassword'));
                if ($password === get_user_caiwu_info($uid, 'paypassword')) {
                    $result = $user_object->where(array('uid' => $uid))
                            ->setField('paypassword', $newpassword);
                    if ($result) {
                        $this->success('支付密码修改成功');
                    } else {
                        $this->error('支付密码修改失败'.$user_object->getError());
                    }
                } else {
                    $this->error('旧密码输入错误');
                }
            ...
            // dump($tab_list);
            $builder = new \Common\Builder\FormBuilder();
            $builder->setMetaTitle('修改支付密码')  // 设置页面标题
                    ->setTabNav($tab_list,2)  // 设置Tab按钮列表 
                    ->setPostUrl(U(''))        // 设置表单提交地址
                    ->addFormItem('password', 'password', '旧支付密码')
                    ->addFormItem('newpassword', 'password', '新支付密码')
                    ->addFormItem('repassword', 'password', '重复新支付密码')
                    ->setTemplate(C('USER_CENTER_FORM'))
                    ->display();
    ...
    public function profile() {
        if (IS_POST) {
            ...
            // 保存昵称,身份证号码，手机号
            $udata['nickname']= I('post.nickname');
            $udata['id_number']= I('post.id_number');
            $udata['mobile']= I('post.mobile');
            if ($udata) {
                $result = $user_object->where(array('id' => $uid))->setField($udata);
                if ($result === false) {
                    $this->error('修改失败'.$user_object->getError());
                }
            } else {
                $this->error('请填写昵称、身份证号码、手机号');
            }
            ...
            if ($attribute_list[$type]) {
                ...
                $mobile['title'] = '手机号码';
                $mobile['type']  = 'text';
                $mobile['value']  = $user_info['mobile'];
                //增加身份证号码表单
                $id_number['name']  = 'id_number';
                $id_number['title'] = '身份证号码';
                $id_number['type']  = 'text';
                $id_number['value']  = $user_info['id_number'];
                $new_attribute_list[1][0] = $nick;
                $new_attribute_list[1][10] = $mobile;
                $new_attribute_list[1][11] = $id_number;
                // var_dump($new_attribute_list);
            ...
            // print_r($new_attribute_list_sort);
            // 使用FormBuilder快速建立表单页面。
            $builder = new \Common\Builder\FormBuilder();
